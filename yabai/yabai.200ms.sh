...
# Now sets colours acording to state of window_topmost

TOPMOSTOFFCOLOR=$RED
TOPMOSTONCOLOR=$YELLOW
TOPMOSTOFFDEFAULT=$WHITE
TOPMOSTONDEFAULT=$MAGENTA
...
TOPMOST=$(yabai -m config window_topmost)
if [[ $TOPMOST == "on" ]]; then
    COLOR=$TOPMOSTONCOLOR
    DEFAULT=$TOPMOSTONDEFAULT
    LEFT=$TOPMOSTONLEFT
    RIGHT=$TOPMOSTONRIGHT
else
    COLOR=$TOPMOSTOFFCOLOR
    DEFAULT=$TOPMOSTOFFDEFAULT
    LEFT=$TOPMOSTOFFLEFT
    RIGHT=$TOPMOSTOFFRIGHT
fi
...
if [[ $WINTOPSTATUS == "3" ]]; then
    ...
    WINTOPSTATUS=$WINTOPFALSE
fi

if [[ $WINTOP = true ]]; then
    FINAL=$FINALSTRING$SPACELEFT$TYPE$SPACERIGHT$WINLEFT$WINDOW$WINRIGHT$TOPLEFT$WINTOPSTATUS$TOPRIGHT
elif [[ "$SPACETYPE" = true ]] && [[ "$WINTYPE" = true ]]; then
